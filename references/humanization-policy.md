...
For full sections or papers, use `../scripts/check_prose_quality.py` for candidate locations. Fix real problems; keep justified scientific language even when a pattern matches. A clean script result alone does not establish natural prose, correctness, or publication readiness.
