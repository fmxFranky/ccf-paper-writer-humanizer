...
Use with `score-lifting-loop.md` and, only when explicitly requested or allowed by the CCFA handoff mode, `ccf-paper-reviewer` for scientific or writing review. Use `ccf-literature-searcher`, `ccf-experiment-designer`, or `ccf-paper-writer-humanizer-humanizer` compression mode through the same handoff rule when the blocker is missing prior art, evidence design, or page-limit pressure.
